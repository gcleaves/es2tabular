...
class DuckDBService {
  ...
  async query(sql) {
    ...
    try {
      ...
      // Convert BigInt and DuckDB date/timestamp values for JSON serialization
      // DuckDB returns DuckDBDateValue (has .days) and DuckDBTimestampValue (has .micros) - custom types, not native Date
      const serializedRows = rows.map(row => {
        ...
        for (const [key, value] of Object.entries(row)) {
          if (typeof value === 'bigint') {
            ...
            ('days' in value || 'micros' in value || 'millis' in value || 'nanos' in value)) {
            // DuckDB DATE/TIME/TIMESTAMP/INTERVAL types - use toString() for human-readable output
